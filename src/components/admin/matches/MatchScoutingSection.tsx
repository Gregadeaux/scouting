...
function TeamScoutingCard({
  ...
  // Check if this is average data (SCOUT-55)
  const isAverageData = entries.length === 1 && entries[0].auto_performance?.is_average === true;
  const matchCount = isAverageData ? (entries[0].auto_performance?.match_count || 0) : 0;
  const hasNoData = isAverageData && matchCount === 0;
    ...
    <Card className={`border-2 ${allianceColor}`}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {hasNoData ? (
            <Badge variant="danger" className="bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300">
              No prior data
            </Badge>
          ) : isAverageData ? (
            <Badge variant="secondary" className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300">
              Avg from {matchCount} {matchCount === 1 ? 'match' : 'matches'}
            </Badge>
          ) : (
      ...
      <CardContent className="pt-4">
        {hasNoData ? (
          <div className="text-center py-4 text-sm text-gray-500">
            No previous match data available for this team at this event.
          </div>
        ) : avgMetrics ? (
          <div className="grid grid-cols-4 gap-2 text-center">
            <div>
              <div className="text-xs text-gray-600 dark:text-gray-400">{isAverageData ? 'Avg Auto' : 'Auto'}</div>
              <div className="text-lg font-semibold">{avgMetrics.auto.toFixed(1)}</div>
            </div>
            <div>
              <div className="text-xs text-gray-600 dark:text-gray-400">{isAverageData ? 'Avg Teleop' : 'Teleop'}</div>
              <div className="text-lg font-semibold">{avgMetrics.teleop.toFixed(1)}</div>
            </div>
            <div>
              <div className="text-xs text-gray-600 dark:text-gray-400">{isAverageData ? 'Avg Endgame' : 'Endgame'}</div>
              <div className="text-lg font-semibold">{avgMetrics.endgame.toFixed(1)}</div>
            </div>
            <div>
              <div className="text-xs text-gray-600 dark:text-gray-400">{isAverageData ? 'Avg Total' : 'Total'}</div>
          ...
          <div className="mt-4 space-y-3 border-t pt-4">
            {isAverageData ? (
              <div className="text-center text-sm text-gray-500 py-2">
                These values are calculated from {matchCount} previous {matchCount === 1 ? 'match' : 'matches'} at this event.
                {matchCount === 0 && ' No prior match data available for this team.'}
              </div>
            ) : (
